...
class AdvModelSaliencyTCCNet(AdvModel, ABC):
    ...
    def get_adv_regs(self, att_base: Tuple, att_adv: Tuple) -> Dict:
        if self.__sal_type == "spat":
            return self.get_adv_spat_loss(att_base[0], att_adv[0])

        if self.__sal_type == "temp":
            return self.get_adv_temp_loss(att_base[1], att_adv[1])
